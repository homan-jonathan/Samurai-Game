...
public class WMEnemySightScript : MonoBehaviour
{
    ...
    WMSightIndicatorScript _sightIndicatorScript;
    WMGuardSoundsScript _guardSoundScript;
    AlertGuardScript _alertGuardScript;
    ...
    void Start()
    {
        _sightIndicatorScript = GetComponentInChildren<WMSightIndicatorScript>();
        _guardSoundScript = GetComponent<WMGuardSoundsScript>();
        _playerTransform = GetComponent<WMEnemyAnimScript>().GetPlayerReference().transform;
        _playerMoveScript = GetComponent<WMEnemyAnimScript>().GetPlayerReference().GetComponent<PlayerMoveScript>();
        _alertGuardScript = GetComponentInChildren<AlertGuardScript>();
    }
    ...
    public bool CanSeePlayer()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(ray, out hit, CalculateViewDistance() * _sightIndicatorScript.GetVisibilityInicatorRange()))
        {
            if ((!(Vector3.Angle(transform.forward, directionToPlayer) < WARNING_VIEW_ANGLE/2) ||
                hit.collider.tag != Tag.player) &&
